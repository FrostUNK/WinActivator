...
$form = New-Object System.Windows.Forms.Form -Property @{Text = "WinActivator: Choosing application for activation"; Width = 400; Height = 150}
...
switch ($ver) {
    "Windows 10/11" {
        $tmpFile = [System.IO.Path]::Combine($env:Temp, 'WinActivator.bat')
        Set-Content $tmpFile -Value "@echo off
        title WinActivator: $ver
    ...
    "Office 2013" {
        ...
        elseIf (test-path "C:\Program Files (x86)\Microsoft Office\Office15\ospp.vbs") {
            ...
            [System.Windows.Forms.MessageBox]::Show("$ver not found!","WinActivator",[System.Windows.Forms.MessageBoxButtons]::OK,[System.Windows.Forms.MessageBoxIcon]::Error)
            exit
        }
        New-Item -ItemType Directory -Path "$env:temp\WinActivator"
        Invoke-WebRequest -Uri "https://github.com/FrostUNK/WinActivator/raw/main/files/Office_2013_Library.zip" -OutFile $env:temp\WinActivator\library.zip
        Expand-Archive -Path "$env:temp\WinActivator\library.zip" -DestinationPath "$env:temp\WinActivator\"  
        Set-Content $tmpFile -Value "@echo off
        title WinActivator: $ver
        Echo Activating $ver, please wait...
        cd /d %temp%\WinActivator\library
    ...
    "Remove all Office16 licenses" {
        ...
        elseIf (test-path "C:\Program Files (x86)\Microsoft Office\Office16\ospp.vbs") {
            ...
            [System.Windows.Forms.MessageBox]::Show("Office16 folder not found!","WinActivator",[System.Windows.Forms.MessageBoxButtons]::OK,[System.Windows.Forms.MessageBoxIcon]::Error)
            exit
        }
        Start-Process -FilePath powershell.exe -ArgumentList "-Command `"`$host.ui.RawUI.WindowTitle = 'WinActivator: Removing all Office16 licenses';cd '$path';while(`$true){`$license = (cscript ospp.vbs /dstatus) | Out-String;`$match = `$license | Select-String -Pattern 'Last 5 characters of installed product key: (\w{5})';if (`$match) {`$productKey = `$match.Matches.Groups[1].Value;cscript ospp.vbs /unpkey:`$productKey}else{exit}}`"" -Verb RunAs -Wait
        [System.Windows.Forms.MessageBox]::Show("All Office16 licenses have been removed.","WinActivator",[System.Windows.Forms.MessageBoxButtons]::OK,[System.Windows.Forms.MessageBoxIcon]::Information)
        exit
        }
}
...
if ($ver -like "* *") {
    ...
    [System.Windows.Forms.MessageBox]::Show("Ok, I did activate nothing.","WinActivator",[System.Windows.Forms.MessageBoxButtons]::OK,[System.Windows.Forms.MessageBoxIcon]::Information)
    exit
}